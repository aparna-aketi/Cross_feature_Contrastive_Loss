...
class GossipDataParallel(Module):
    ...
    def __init__(self, module, device_ids=None, rank=None, world_size=None,
                 ...
                 gamma = 1.0, momentum=0.9, lr=0.1, weight_decay = 0, qgm=0, nesterov=False):
...
##############################################################
        ...
        if not self.is_ps_numerator:
            ...
            for p, m in zip(self.module.parameters(), self.momentum_buff):
                if p.requires_grad:
                    ...
                    m.data.mul_(self.momentum).add_(p.grad.data)
                    if self.nesterov:
                        p.grad.data.add_(m.data, alpha = self.momentum)
                    else: 
                        p.grad.data.copy_(m.data)
    ...
    def _query_gossip_queue(self, non_blocking=False):
        ...
        if self.gossip_flag.is_set():
            ...
            for p, r, m in zip(self.module.parameters(), self.gossip_device_buffer, self.momentum_buff):
                ...
                p.data.add_(r) 
                if self.qgm==1:
                    m.data.add_(r.data, alpha=(-1.0/(self.dist_config['lr_prev']*float(self.ps_weight.data))))   
                   
            self.dist_config['lr_prev'] = self.lr
            # update flags
    ...
    def _gossip_into_receive_buffer(send_buffer, gossiper, receive_buffer,
        ...
        send_msg  = flatten_tensors(send_buffer)
        # send and receive parameters
        with gossip_lock:
            data_amt = 0
            in_msg, updated_ps_weight, neigh_weights, data_amt = gossiper.mix(send_msg, send_buffer, ps_weight, residual=True)
